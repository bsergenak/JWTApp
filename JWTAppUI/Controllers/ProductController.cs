...
namespace JWTAppUI.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> List()
        {
            ...
            HttpResponseMessage response = await client.GetAsync("http://localhost:7055/api/Products");
            if (response.IsSuccessStatusCode)
            {
                ...
                foreach (var item in list)
                {
                    ...
                    var responseCategory = await client.GetAsync($"http://localhost:7055/api/Categories/{item.CategoryId}");
                    ...
                    var responseSupplier = await client.GetAsync($"http://localhost:7055/api/Suppliers/{item.SupplierId}");
        ...
        {
            HttpClient client = CreateClient();
            HttpResponseMessage response = await client.GetAsync("http://localhost:7055/api/Categories");
            if (response.IsSuccessStatusCode)
            {
                //GetCategories
                string jsonString = await response.Content.ReadAsStringAsync();
        ...
        {
            //GetSuppliers
            HttpResponseMessage responseSuppliers = await client.GetAsync("http://localhost:7055/api/Suppliers");
        ...
        public async Task<IActionResult> Create(ProductCreateRequestModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                HttpResponseMessage response = await client.PostAsync("http://localhost:7055/api/Products", requestContent);
        ...
        public async Task<IActionResult> Update(int id)
        {
            ...
            var response = await client.GetAsync("http://localhost:7055/api/Products/" + id);
            if (response.IsSuccessStatusCode)
            {
                ...
                var responseCat = await client.GetAsync("http://localhost:7055/api/Categories");
        ...
        public async Task<IActionResult> Update(ProductUpdateRequestModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var response = await client.PutAsync("http://localhost:7055/api/Products", requestContent);
        ...
        {
            var client = this.CreateClient();
            await client.DeleteAsync($"http://localhost:7055/api/Products/{id}");
            return RedirectToAction("List");
        }
    }
}